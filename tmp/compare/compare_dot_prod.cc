...
template
void compare_dot_prod_fma<double>(int n,double required_cond, int nb_gen, double sum, std::vector<double> &Time, std::vector<mpfr_t> &Error, int q, int nb_threads);
...
/// @param q file position (main/X/ = 2   main/X/Y/ = 1)
template < class T >
void compare_dot_prod_fma(int n,double required_cond, int nb_gen,  double sum, std::vector<T> &Time, std::vector<mpfr_t> &Error, int q,int nb_threads){
...
template
void compare_dot_prod_vec<double>(int n,double required_cond, int nb_gen, double sum, std::vector<double> &Time, std::vector<mpfr_t> &Error, int q, int nb_threads);
...
/// @param q file position (main/X/ = 2   main/X/Y/ = 1)
template < class T >
void compare_dot_prod_vec(int n,double required_cond, int nb_gen,  double sum, std::vector<T> &Time, std::vector<mpfr_t> &Error, int q,int nb_threads){
    ...
    res_par_standard = 0.0; 
    #pragma omp parallel for simd reduction(+:res_par_standard)   // static    dynamic      guided     auto     runtime
    for (unsigned int j=0; j<n;j++){
        res_par_standard += a[j]*b[j];
    }
    ...
    for (unsigned int t=0; t<NB_EXEC;t++){
        ...
        #pragma omp parallel for simd reduction(+:res_par_standard) 
        for (unsigned int j=0; j<n;j++){
            res_par_standard += a[j]*b[j];
        }
        clock_gettime(CLOCK_REALTIME,&Min_end_par_standard);
