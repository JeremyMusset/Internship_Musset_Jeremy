...
int main() {
    ...
    seq_dot_prod(n, cond, nb_gen,sum,Time_seq,Error_seq,sz);
    par_dot_prod(n, cond, nb_gen,nb_threads,sum,Time_par,Error_par,sz);
    
    // Print
    printf("\nTime sequential mpfr : %.30f\n",Time_seq[0]);
    printf("Time sequential common dot prod : %.30f \n",Time_seq[1]);
    printf("Time sequential rare blas dot prod : %.30f\n",Time_seq[2]);

    printf("\nTime parallel mpfr : %.30f\n",Time_par[0]);
    printf("Time parallel common dot prod : %.30f \n",Time_par[1]);
    printf("Time parallel rare blas dot prod : %.30f\n",Time_par[2]);

    mpfr_printf("\nError sequential common dot prod : %.30Rg \n",Error_seq[1]);
    mpfr_printf("Error sequential rare blas dot prod : %.30Rg \n\n",Error_seq[2]);

    mpfr_printf("\nError parallel common dot prod : %.30Rg \n",Error_par[1]);
    mpfr_printf("Error parallel rare blas dot prod: %.30Rg \n\n",Error_par[2]);
